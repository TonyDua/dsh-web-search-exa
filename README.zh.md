...
> **无需 API key**：一个 `ctx.web` seam 的 `WebSearchProvider`，内置匿名 MCP 兜底和带 key 的 REST 路径。

使用 [deepseek-v4-flash](https://api-docs.deepseek.com) 在 DeepSeek Harness（dsh）内开发。

## 当前支持的版本

`0.1.2-alpha.2` 到 `0.1.7-alpha.1` 之间**每一个**已发布的 dsh 版本都实测过。实测指三件事：独立安装该版本、用该版本自己的类型声明做类型检查、并用 npm 严格安装一次本插件。最后一步最容易失败，因为 npm 的 peer 规则比 pnpm 严。复现命令：`bash scripts/compat-matrix.sh`。
...
**（1）`0.1.7-alpha.1` 换掉了 settings API。** `SettingsProvider.installSection` 被移除，服务变成 `SettingsForms`，它直接从 Loader 已持有的 Config schema 派生配置页（`SettingsDescriptor.schema`、`autoGenerate`）。旧代码无条件调用该方法，会在这个版本上抛 `TypeError`：插件能加载，但会失败。现在改为先探测方法，存在才调用，不存在则什么都不做。在 `0.1.7+` 上由 Loader 的 schema 驱动表单，插件无需注册任何东西。

**（2）`0.1.7-alpha.1` 依赖 `@deepseek-ai/cordis` `^4.0.3`**，而 cordis 的 `latest` dist-tag 仍指向 `4.0.2`。`4.0.3` 已发布，只是 tag 落后。搭配 `0.1.7` 宿主时请安装 `@deepseek-ai/cordis@4.0.3`。矩阵脚本已按版本固化这一点。
...
- 匿名通道的 429 抛出 `WEB_RATE_LIMITED`，而不是笼统的 `WEB_PROVIDER_ERROR`，错误信息里直接点名 `EXA_API_KEY`。
- 配了 key 的 REST 路径不受熔断影响。付费端点的失败应当让你看到。

**但“是否真的自动回退”取决于 harness 侧。** seam 只会选中唯一一个可用 provider，并不存在优先级链。同时存在多个可用 provider 时，它会抛 `WEB_PROVIDER_AMBIGUOUS`。因此：
...
- 🆓 **零配置、默认免 key**：搜索经由 Exa 官方托管的 MCP 服务器（`mcp.exa.ai/mcp`），完全不携带凭据。这是 Exa 官方提供的免认证公共 MCP，有限流。
- 🔑 **配 key 自动升级 REST**：设置 `EXA_API_KEY` 后自动切换到 Exa `POST /search` REST API，额度更高，行为不变。
- 🔌 **即插即用**：注册进 dsh `ctx.web` seam。模型侧的 `web_search` 和 `web_fetch` 工具、提示词区段与结果卡片都无需改动。
- 🎛️ **`providerId` 开关**：可与官方 `@deepseek-ai/dsh-web-search-exa` 在同一个 profile 共存，不撞 id，也没有黑箱覆盖。
...
- 你有 `EXA_API_KEY`，且想用官方维护的包：用 `@deepseek-ai/dsh-web-search-exa`，它是官方标准实现。
- 想零配置、免 key 试用 Exa 搜索：用本包。默认走匿名 MCP，出现 key 后自动走 REST。
- 两个都想要：一起装，用 `providerId` 开关区分，见[与官方包共存](#与官方包共存)。
...
匿名 MCP 路径不发送任何凭据，来源标识通过 `x-exa-source: dsh-anything` 头携带。结果按 seam 的 `WebSearchSource` 形状规范化（`url`、`title`、`snippet`、`publishedAt`），`maxResults` 由 seam 在返回路径上强制执行。匿名使用受 Exa 限流，HTTP 429 会以独立的 `WEB_RATE_LIMITED` 码呈现，并提示配置 API key。

## 安装

> **同一个产物，两个入口。** CI 打包本版本的 tarball，在每一个受支持的 dsh 版本上验证，挂到 GitHub Release，并把**这个产物本身**发布到 npm。所以 Release 附件和 npm 上的 tarball 是同一个文件，而不是两次恰好一致的构建。

三种方式选一种。方式只影响代码从哪来，不影响后面的配置。
...
装完执行 `dsh web --help` 或重启 `dsh web` 生效。模型侧的 `web_search` 工具随即走本提供方，不需要改工具配置。

### 选中提供方

无 API key 时，官方 DeepSeek 搜索提供方不可用，seam 会自动选中本插件，不需要改配置。

配置了 key 时，两个 provider 都可能可用，你需要显式选中一个。二选一：
...
`@deepseek-ai/dsh-tools` 是 dsh 的运行时单例包，一个 profile 中必须解析到同一份物理包实例。本插件不依赖它，这是宿主 profile 的依赖约束。

如果 profile 里的其他第三方插件把它错误声明成普通嵌套依赖，而不是 peer dependency，dsh agent loop 可能在 provider 被调用前就因 `Cannot read properties of undefined (reading 'prepare')` 失败。此时先修正那个插件的依赖声明，或让 profile 的包管理器统一解析到共享实例，再排查搜索错误。
...
- **现在怎么改配置**：编辑 `$DSH_HOME/profiles/web/cordis.patch.yml` 里本插件的 `config`，字段与默认值见上方配置表，然后重启 `dsh web`。也可以用环境变量 `EXA_API_KEY` 或 `$DSH_WEB_SEARCH_PROVIDER`。`apiKey` 标记了 `role('secret')`，任何 `describe()` 响应都不会暴露它的值。
- **搜索结果卡片**：`web_search` 调用经 `dsh-tool-web` 照常渲染 `web` 结果卡片（来源、摘要、日期），与提供方无关。匿名 Exa 的结果和 DeepSeek 搜索显示一致。

**路线图（下一版本）**：新增注册到 `settings.plugin.item` slot 的客户端卡片，绑定 `web-search-exa` 命名空间，让上表所有字段可以在 Settings → Plugins 里实时编辑。
...
**Q：遇到 HTTP 429 或限流怎么办？**

这是 Exa 匿名 MCP 的限流。配置 `EXA_API_KEY`，或在 `apiKey` 字段写入密钥，提供方会自动切到 REST 路径。
...
`0.1.2-alpha.2` 到 `0.1.7-alpha.1` 之间每一个已发布的版本都实测过。peer 范围里的 `>=0.1.8` 用来承接之后的稳定版，但这些版本尚未实测。实测方式见[为什么 peer 范围长这样](#为什么-peer-范围长这样)。
...
同时感谢 **[Exa](https://exa.ai)** 提供并运营这个免费、免认证的托管 MCP 服务器（`mcp.exa.ai/mcp`），正是它让本包的零配置默认路径成为可能。Exa 托管 MCP 是 Exa 的官方产品，匿名使用有限流，见 FAQ。
