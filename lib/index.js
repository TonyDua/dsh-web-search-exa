...
var ExaSearchProvider = class {
	...
	*/
	constructor(resolveOptions, resolveApiKey = resolveApiKeyFromProcess) {
		...
		this.id = resolveOptions().providerId ?? "exa";
	}
	...
	async search(request, signal) {
		...
		return apiKey !== void 0 ? await this.#restSearch(request, apiKey, options, signal) : await this.#anonymousMcpSearch(request, options, signal);
	}
	...
	*/
	async #anonymousMcpSearch(request, options, signal) {
		...
		if (!response.ok) {
			if (response.status === 429) throw new WebError("Exa anonymous MCP rate limit reached (HTTP 429); configure an EXA_API_KEY for higher limits", "WEB_PROVIDER_ERROR");
		...
		if (payload === null) throw new WebError("Exa anonymous MCP returned an unprocessable response body", "WEB_PROVIDER_ERROR");
...
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_API_URL, DEFAULT_BASE_URL, DEFAULT_HIGHLIGHTS_PER_RESULT, DEFAULT_MCP_URL, DEFAULT_PROVIDER_ID, DEFAULT_SEARCH_TYPE, ExaSearchProvider, MAX_SNIPPET_CHARS, MCP_SOURCE, MCP_TOOL, PROVIDER_ID, SETTINGS_NAMESPACE, USER_AGENT, apply, inject, name, resolveApiKey, resolveApiKeyFromProcess, resolveOptions };
